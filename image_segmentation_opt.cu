...
/* TODO: il codice funziona, ma fa troppe volte l'add nella sum
         cioè: vengono duplicate delle somme */
...
__global__ void sum_kernel(const float3 *img, const size_t width, const size_t height, const int *associations, float4 *result)
{
    ...
    atomicAdd(&(sums[associations[i]].x), img[i].x);
    atomicAdd(&(sums[associations[i]].y), img[i].y);
    atomicAdd(&(sums[associations[i]].z), img[i].z);
    atomicAdd(&(sums[associations[i]].w), 1);
    ...
    if (threadIdx.x == 0 && threadIdx.y == 0)
    {
        ...
        for (int j = 0; j < K; j++)
        {
            atomicAdd(&(result[j].x), sums[j].x);
            atomicAdd(&(result[j].y), sums[j].y);
            atomicAdd(&(result[j].z), sums[j].z);
            atomicAdd(&(result[j].w), sums[j].w);
            sum_w += sums[j].w;
        }
...
int main(int argc, char **argv)
{
    ...
    while (!done && iteration < MAX_ITERATION)
    {
        ...
        sum_kernel<<<gridDim, blockDim, k * sizeof(float4)>>>(device_img, width, height, device_associations, device_centroid_sums);
